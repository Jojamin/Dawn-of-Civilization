...
def flipCity(tCityPlot, bConquest, bKillUnits, iNewOwner, lOldOwners = []):
	...
	flipCity = city(tCityPlot)
	
	if not flipCity:
		raise Exception("no city there to be flipped")
...
def colonialAcquisition(iPlayer, tPlot):
	...
	iMissionary = missionary(player(iCiv).getStateReligion())
	if iMissionary:
...
def getBorderPlots(iPlayer, tTL, tBR, iDirection = DirectionTypes.NO_DIRECTION, iNumPlots = 1):
	...
	return [getPlotNearCityInDirection(city, iDirection) for city in lTargetCities]
...
def getPlotNearCityInDirection(city, iDirection):
	...
	return secondRing.where(estimate_direction(city, p) == iDirection).random()
...
def isIsland(plot, iIslandLimit = 3):
	return map.getArea(plot(plot).getArea()).getNumTiles <= iIslandLimit
...
def checkSlaves(iPlayer):
	if not player(iPlayer).canUseSlaves():
		...
		for slave in units.owner(iPlayer).where(lambda u: base_unit(unit) == iSlave):
...
def relocateUnitsToCore(iPlayer, lUnits):
	...
	dUnits = units.of(lUnits).where(lambda unit: unit.plot().getOwner() not in [iPlayer, -1]).by_type()
