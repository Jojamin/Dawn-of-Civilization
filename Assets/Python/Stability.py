	...
	for city in lCities:
		if bRazeMinorCities:
			...
			if bMaxPopulation and bMaxCulture and bNoHolyCities and bNoCapitals and bNotJerusalem:
				closestCity = map.findCity(city.getX(), city.getY(), iPlayer, -1, True, False, -1, -1, city)
			...
			for iLoopPlayer in range(iNumPlayers):
				...
				if pLoopPlayer.isAlive() and tPlayer.isAtWar(iLoopPlayer) and not player(iLoopPlayer).isHuman() and year() < year(tFall[iLoopPlayer]):
					if pLoopPlayer.getWarValue(city.getX(), city.getY()) >= 8 and team(iLoopPlayer).AI_getWarSuccess(iPlayer) > tPlayer.AI_getWarSuccess(iLoopPlayer):
						...
						closestCity = map.findCity(city.getX(), city.getY(), PlayerTypes.NO_PLAYER, TeamTypes.NO_TEAM, True, False, TeamTypes.NO_TEAM, DirectionTypes.NO_DIRECTION, city)
