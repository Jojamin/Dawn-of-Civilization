...
#############
	...
	def eventApply7629(self, netUserData, popupReturn):
		...
		closestCity = map.findCity(unit.getX(), unit.getY(), iByzantium, TeamTypes.NO_TEAM, False, False, TeamTypes.NO_TEAM, DirectionTypes.NO_DIRECTION, CyCity())
		
		newUnit = makeUnit(iByzantium, unit.getUnitType(), closestCity).first()
...
#####################################  
	...
	def adjust1700ADCulture(self):
		...
		for x, y in [(48, 45), (50, 44), (50, 43), (50, 42), (49, 40)]:
			convertPlotCulture(plot(x, y), iPortugal, 100, True)
			
		for x, y in [(58, 49), (59, 49), (60, 49)]:
			convertPlotCulture(plot(x, y), iGermany, 100, True)
			
		for x, y in [(62, 51)]:
			convertPlotCulture(plot(x, y), iHolyRome, 100, True)
			
		for x, y in [(58, 52), (58, 53)]:
			convertPlotCulture(plot(x, y), iNetherlands, 100, True)
			
		for x, y in [(64, 53), (66, 55)]:
			convertPlotCulture(plot(x, y), iPoland, 100, True)
			
		for x, y in [(67, 58), (68, 59), (69, 56), (69, 54)]:
			convertPlotCulture(plot(x, y), iRussia, 100, True)
	...
	def prepareChina(self):
		...
		if scenario() == i600AD:
			...
			pBeijing = plot(tBeijing, tBeijing)
			city(pBeijing).kill()
		...
		for plot in plots.start(tCultureRegionTL).end(tCultureRegionBR):
			bCity = False
			for loopPlot in plots.surrounding(pPlot):
				if loopPlot.isCity():
					bCity = True
	...
	def rebirthSecondTurn(self, iCiv):
		...
		lCities = [city(x, y) for x, y in lRebirthPlots if city(x, y)]
	...
	def birthInCapital(self, iCiv, iPreviousOwner, tCapital, tTopLeft, tBottomRight):
		...
		if data.players[iCiv].iFlipsDelay == 0:
			...
			if iFlipsDelay > 0:
				...
				if not plot_(tCapital).isCity():
					raise Exception("no city here even though it should")

				# flip capital instead of spawning starting units
				flipCity(tCapital, False, True, iCiv, ())
				city(x, y).setHasRealBuilding(iPalace, True)
				convertPlotCulture(plot_(x, y), iCiv, 100, True)
				self.convertSurroundingPlotCulture(iCiv, plots.surrounding(tCapital))
	...
	def giveColonists(self, iCiv):
		...
		if player(iCiv).isAlive() and not player(iCiv).isHuman() and iCiv in dMaxColonists:
			if teamCiv.isHasTech(iExploration) and data.players[iCiv].iColonistsAlreadyGiven < dMaxColonists[iCiv]:
				cities = cities.rectangle(Areas.getCoreArea(iCiv)).owner(iCiv)
				...
				if iCiv == iEngland:
					...
					if colonialCities:
						cities = colonialCities
						
				city = cities.where(lambda city: city.isCoastal(10)).random()
				if city:
			...
			if iTeamX in lCivBioNewWorld and iHasMetTeamY in lCivBioOldWorld:
				...
				if not bAlreadyContacted:
					...
					if contactPlots and arrivalPlots:
						...
						if iInfantry:
							makeUnits(iOldWorldCiv, iInfantry, tArrivalPlot, 1 + iModifier2, UnitAITypes.UNITAI_ATTACK_CITY)
						
						if iCounter:
							makeUnits(iOldWorldCiv, iCounter, tArrivalPlot, 2, UnitAITypes.UNITAI_ATTACK_CITY)
							
						if iSiege:
							makeUnits(iOldWorldCiv, iSiege, tArrivalPlot, 1 + iModifier1 + iModifier2, UnitAITypes.UNITAI_ATTACK_CITY)
							
						if iCavalry:
							makeUnits(iOldWorldCiv, iCavalry, tArrivalPlot, 2 + iModifier1, UnitAITypes.UNITAI_ATTACK_CITY)
							
						if iMissionary:
							makeUnit(iOldWorldCiv, iMissionary, tArrivalPlot)
							
						if iNewWorldCiv == iInca:
							makeUnits(iOldWorldCiv, iAucac, tArrivalPlot, 3, UnitAITypes.UNITAI_ATTACK_CITY)
						elif iNewWorldCiv == iAztecs:
							makeUnits(iOldWorldCiv, iJaguar, tArrivalPlot, 2, UnitAITypes.UNITAI_ATTACK_CITY)
							makeUnit(iOldWorldCiv, iHolkan, tArrivalPlot, UnitAITypes.UNITAI_ATTACK_CITY)
						elif iNewWorldCiv == iMaya:
							makeUnits(iOldWorldCiv, iHolkan, tArrivalPlot, 2, UnitAITypes.UNITAI_ATTACK_CITY)
							makeUnit(iOldWorldCiv, iJaguar, tArrivalPlot, UnitAITypes.UNITAI_ATTACK_CITY)
		...
		for iTargetCiv in targetCivList:
			if player(iTargetCiv).isHuman():
				...
				for tPlot in targetList:
					if city(tPlot).getOwner() == iTargetCiv:
						iNumCities += 1
				...
				for tPlot in targetList:
					if plot(tPlot).getPlotCity().getOwner() == iTargetCiv:
						if bAccepted:
							colonialAcquisition(iPlayer, tPlot)
