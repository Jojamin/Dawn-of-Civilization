...
def getUHVTileInfo(argsList):
	...
	if iPlayer == iGreece:
		...
		if plot in plots.start(vic.tNWIndiaTL).end(vic.tNWIndiaBR).without(vic.tNWIndiaExceptions):
			return 6
		...
		if plot in plots.start(vic.tCanadaWestTL).end(vic.tCanadaWestBR).without(vic.tCanadaWestExceptions) or plot in plots.start(vic.tCanadaEastTL).end(vic.tCanadaEastBR).without(vic.tCanadaEastExceptions):
			return 62
