...
def startsGlobalWar(attackers, defenders):
	...
	return 2 * participatingPowers >= worldPowers
...
class Congress:
	...
	def applyIntroductionEvent(self):
		...
		for iLoopPlayer in self.invites:
			if not player(iLoopPlayer).isAlive(): lRemove.append(iLoopPlayer)
			
		for iLoopPlayer in lRemove:
			self.invites.remove(iLoopPlayer)
	...
	def startBriberyEvent(self, iVoter, iClaimant, (x, y), iDifference, iClaimValidity):
		...
		if bHumanClaim:
			if bCity:
				...
				closestCity = map.findCity(x, y, iBribedPlayer, TeamTypes.NO_TEAM, True, False, TeamTypes.NO_TEAM, DirectionTypes.NO_DIRECTION, CyCity())
				sText = text("TXT_KEY_CONGRESS_BRIBE_OWN_CLAIM_COLONY", adjective(iBribedPlayer), adjective(plot), closestCity.getName())
		else:	
			if bCity:
				sText = text("TXT_KEY_CONGRESS_BRIBE_OWN_CITY", adjective(iBribedPlayer), adjective(iClaimant), plot.getPlotCity().getName())
			else:
				closestCity = map.findCity(x, y, human(), TeamTypes.NO_TEAM, True, False, TeamTypes.NO_TEAM, DirectionTypes.NO_DIRECTION, CyCity())
				sText = text("TXT_KEY_CONGRESS_BRIBE_OWN_TERRITORY", adjective(iBribedPlayer), adjective(iClaimant), closestCity.getName())
	...
	def startRefusalEvent(self, iClaimant, (x, y)):
		...
		if plot.isCity():
			...
			closestCity = map.findCity(x, y, human(), TeamTypes.NO_TEAM, True, False, TeamTypes.NO_TEAM, DirectionTypes.NO_DIRECTION, CyCity())
			sText = text("TXT_KEY_CONGRESS_DEMAND_TERRITORY", name(iClaimant), closestCity.getName(), sVotedYes)
	...
	def selectClaims(self, iPlayer):
		...
		for iLoopPlayer in range(iNumTotalPlayers+1):
			...
			for city in cities.owner(iLoopPlayer):
				...
				# close to own empire
				closestCity = map.findCity(x, y, iPlayer, TeamTypes.NO_TEAM, False, False, TeamTypes.NO_TEAM, DirectionTypes.NO_DIRECTION, city)
				iDistance = distance(city, closestCity)
				if iDistance < 5:
					iValue += 5-iDistance
		...
		return sorted(lPlots, lambda plot: plot[2] + random(3), True)[:10]
	...
	def inviteToCongress(self):
		...
		self.invites = self.invites.including(players.major().without(invites).sort(rank))
