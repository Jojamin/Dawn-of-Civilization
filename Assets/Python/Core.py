...
def text(key, *format):
	if isinstance(key, unicode) and len(format) == 0:
		return key
	return translator.getText(str(key.encode('utf-8')), tuple([str(f) for f in format]))
	
	
def text_if_exists(key, format=(), otherwise=None):
...
class EntityCollection(object):
	...
	def including(self, *keys):
		if len(keys) == 1 and isinstance(keys[0], type(self)):
			keys = keys[0]._keys
...
class Plots(EntityCollection):
	...
	def __init__(self, plots):
		super(Plots, self).__init__(plots, plot)
...
class Cities(EntityCollection):
	...
	def __init__(self, plots):
		super(Cities, self).__init__(plots, city)
...
class UnitFactory:
	...
	def at(self, *args):
		if args is None:
			return Units([])
		x, y = _parse_tile(*args)
		units = [unit_key(plot(x, y).getUnit(i)) for i in range(plot(x, y).getNumUnits())]
		return Units(units)
...
class Units(EntityCollection):
	...
	def __init__(self, units):
		super(Units, self).__init__([UnitKey.of(u) for u in units], unit)
...
class Players(EntityCollection):
	...
	def __init__(self, players):
		super(Players, self).__init__(players, lambda x: x)
